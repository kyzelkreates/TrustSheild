...
;(function bootstrapDemoAccess() {
  ...
  try {
    // 1. Setup flag — prevents any setupDone() check redirecting to /auth/setup
    if (localStorage.getItem('apex:setup_complete') !== 'true') {
      localStorage.setItem('apex:setup_complete', 'true')
    }
    // 2. Demo session — write fresh or fix ISO-string expiresAt from old bootstrap
    ...
    // 3. Demo accounts — prevents authService returning empty array
